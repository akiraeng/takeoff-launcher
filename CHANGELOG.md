...
- **Web Search Engine Switcher**: Switch search engines on the fly using `Tab` or keyword prefixes (such as `g`, `d`, `yt`, `gh`, `b`, `w`), with customizable engines in Settings.
- **About Settings Page**: Dedicated About tab featuring hero branding and quick links to GitHub repository, release notes, and license.
