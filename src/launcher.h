...
public:
    bool Create(HINSTANCE instance) {
        ...
        SetTimer(hwnd_, kUpdateCheckTimer, 60 * 60 * 1000, nullptr);
...
private:
    ...
    LRESULT HandleMessage(UINT message, WPARAM wParam, LPARAM lParam) {
        ...
        case WM_TIMER:
            if (wParam == kRenderRetryTimer) {
                ...
                RegisterShortcut();
            } else if (wParam == kUpdateCheckTimer) {
                CheckForUpdatesAsync(false);
        ...
        case WM_DESTROY: {
            ...
            KillTimer(hwnd_, kRenderRetryTimer);
            KillTimer(hwnd_, kTrimTimer);
            KillTimer(hwnd_, kUpdateCheckTimer);
    ...
    void LoadSettings() {
        if constexpr (!kUiTest) {
            ...
            if (settings_.runAtStartup) {
                ...
                RegCloseKey(startup);
                if (settings_.runAtStartup) {
                    std::wstring_view cmd(existingCmd);
                    if (cmd.find(L"--minimized") == std::wstring_view::npos &&
                        cmd.find(L"-minimized") == std::wstring_view::npos &&
                        cmd.find(L"/minimized") == std::wstring_view::npos &&
                        cmd.find(L"--startup") == std::wstring_view::npos &&
                        cmd.find(L"-startup") == std::wstring_view::npos &&
                        cmd.find(L"/startup") == std::wstring_view::npos) {
    ...
    void DrawSettings() {
        ...
        DrawSettingsRow(6, generalTop + 2 * kSettingsRowHeight, L"Check for updates",
            L"Automatically check for updates daily", {}, true, settings_.checkForUpdates);
